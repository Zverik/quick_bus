...
class ItineraryPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: ListView(
        children: [for (var leg in itinerary) ItineraryLeg(leg)],
      ),
    );
  }
}
  ...
  prepareMessages(BuildContext context) {
    ...
    if (widget.leg is TransitRouteElement) {
      ...
      if (element.stopCount <= 3) {
        description = parseTaggedText(loc.rideStops(loc.stops(element.stopCount), destination));
      } else {
        description = parseTaggedText(loc.rideTime(element.arrival, destination));
      }
    } else {
              ...
              layers: [
                ...
                if (widget.leg is TransitRouteElement)
                  CircleLayerOptions(circles: [
                    for (var stop in (widget.leg as TransitRouteElement)
                        .intermediateStops)
                      CircleMarker(
